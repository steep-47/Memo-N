...
globalThis.__memoNSwipeMocks = {
    ...
    repairMissingColumnsBeforeCleanup: options => { repairs.push(options); return []; },
};
...
if (repairs.length !== 1 || repairs[0]?.piece !== piece || repairs[0]?.notify !== false) throw new Error('旧Swipe恢复后未立即执行确定性表头修复');

piece.swipe_id = 0;
handler(0);
if (restored.at(-1)?.state !== 'swipe-0' || piece.memo_n_hash_sheets?.state !== 'swipe-0') throw new Error('切换Swipe后未恢复对应快照');
if (repairs.length !== 2) throw new Error('每次切换Swipe都应重新校验表头结构');

console.log('memo-n-swipe-restore PASS: per-swipe-snapshot=2, post-restore-schema-repair=2, visible-body-preserved=1');
