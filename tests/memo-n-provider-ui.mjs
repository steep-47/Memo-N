...
assert.match(ui, /USER\.tableBaseSetting\.step_by_step = false/u, '用户切换模式后必须清除原插件change handler留下的旧持久step状态');
...
assert.match(modeRuntime, /function preparePromptMode\(\)/u, '必须保留旧index prompt兼容桥');
assert.match(modeRuntime, /function finishPromptMode\(\)\{forceNormalMode\(\);\}/u, 'prompt分发结束必须立即清除旧step标志');
assert.match(modeRuntime, /makeFirst\(promptEvent,preparePromptMode\)/u, '旧step桥必须在prompt监听器最前执行');
assert.match(modeRuntime, /makeLast\(promptEvent,finishPromptMode\)/u, '旧step桥必须在prompt监听器最后归零');
...
assert.match(loader, /RUNTIME_VERSION = 'memon78'/u, 'Loader 缓存版本必须为 memon78');
assert.match(loader, /PUBLIC_VERSION = '0\.1\.0-memon\.78'/u, 'Loader 必须同步公开版本号');
...
assert.equal(manifest.version, '0.1.0-memon.78', 'manifest 版本必须为 memon78');
