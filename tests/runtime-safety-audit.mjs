...
const playerProfileText = await fs.readFile(new URL('../scripts/runtime/playerProfileSchema.js', import.meta.url), 'utf8');
...
if (!contentRulesText.includes('repairMissingColumnsBeforeCleanup({notify:false})') || !(contentRulesText + playerProfileText).includes('稳定外观和持久变化')) throw new Error('角色外貌特征缺少启动迁移或记录语义');
...
if (!finishText.includes('status.noChange===true') || !finishText.includes('Memo-N：七表检查完成，本轮无可写变化')) throw new Error('Memo-N缺少NO_CHANGE明确状态提示');
...
console.log('memo-n engine audit PASS: native-tableedit-one-call=1, final-record-contract=1, last-user-anchor=1, whole-json-disabled=1, stop-preserved=1, reasoning-parser=1, strict-executor=1, session-gate=1, failure-baseline=1, persistence-rollback=1, status-await=1, independent-gates=2, no-change-toast=1');
