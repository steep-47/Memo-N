...
import {rebuildSheets , modifyRebuildTemplate, newRebuildTemplate, deleteRebuildTemplate, exportRebuildTemplate, importRebuildTemplate, triggerStepByStepNow} from "../runtime/absoluteRefresh.js?v=memon4";
...
import {initRefreshTypeSelector} from "../runtime/absoluteRefresh.js?v=memon4";
                ...
                try {
                    ...
                    if (piece) {
                        ...
                        for (let i = chatArr.length - 1; i >= 0; i--) {
                            if (chatArr[i] && Object.prototype.hasOwnProperty.call(chatArr[i], 'memo_n_hash_sheets')) {
                                for (const sheet_id in chatArr[i].memo_n_hash_sheets) {
                                    if (chatArr[i].memo_n_hash_sheets[sheet_id].length > 1) {
                                        isSheetEmpty = false;
                                        break;
                                    }
                                }
                        ...
                        if (!confirmReplace) {
                            ...
                            // 删除聊天列表中所有 piece 的 memo_n_hash_sheets
                            try {
                                for (const msg of chatArr) {
                                    if (msg && Object.prototype.hasOwnProperty.call(msg, 'memo_n_hash_sheets')) {
                                        delete msg.memo_n_hash_sheets;
                                    }
                                }
                            } catch (_) {}
                            // 在当前载体上用全局模板重建
...
function InitBinging() {
    ...
    $('#preset_character_policy').change(function () {
        USER.tableBaseSetting.preset_character_policy = String($(this).val() || 'changes_only');
        USER.saveSettings?.();
        EDITOR.success('预设人物记录策略已更新');
    });
    // 确认执行
...
export function renderSetting() {
    ...
    $('#preset_character_policy').val(USER.tableBaseSetting.preset_character_policy || 'changes_only');
