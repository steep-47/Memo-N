...
- 表4与表5必须指向同一NPC实体；不要因同名就强行合并，也不要因别名变化重复建人。人物事实先绑定明确主体，避免把同段另一人物的年龄、外貌、能力、地点或状态串写进错误人物。
- 6历史事件表：既保留真正影响未来推演的重大既成节点，也保留0～5没有合适字段但后续剧情需要知道的已发生连续性事实，例如异常发现、关键交互、决定、线索、关系转折或连续事件。纯氛围、无后续意义的琐碎动作可删除；同一连续事件优先压缩维护同一条记录，不机械按每轮拆行。
