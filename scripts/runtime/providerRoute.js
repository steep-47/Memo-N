...
export function isOfficialCustomDeepSeek(data) {
    if (sourceOf(data) !== 'custom' || reverseProxyOf(data)) return false;
    const url = customUrlOf(data);
    if (!isOfficialDeepSeekHost(url)) return false;
    const path = pathnameOf(url);
    return path === '' || path === '/v1' || path === '/beta';
}
