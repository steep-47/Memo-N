...
const SENTINEL_BEGIN = 'MEMO_N_EDIT_BEGIN';
...
function summarizeMessage(message) {
    ...
    return {
        ...
        asksMachineBlock: content.includes(SENTINEL_BEGIN) || /tableEdit/i.test(content),
...
function codeFor(summary, outcome = null) {
    ...
    return `DBG P${summary.endpoint.relay ? 1 : 0} T${summary.tablePromptMachine ? 1 : 0} E${summary.contractMachine ? 1 : 0} U${summary.userReminder ? 1 : 0} J${summary.jsonSchemaPresent ? 1 : 0} O${outcome === null ? '?' : outcome ? 1 : 0}`;
}
...
function logRelayRequest(data) {
    ...
    lastRequestSummary = {
        ...
        tablePromptMachine: table.some(item => item.asksMachineBlock),
        contractFound: contract.length > 0,
        contractMachine: contract.some(item => item.asksMachineBlock),
        userReminder: summaries.some(item => item.userReminder && item.asksMachineBlock),
    };
...
function inspectRendered(chatId) {
    ...
    const found = content.includes(SENTINEL_BEGIN) || reasoning.includes(SENTINEL_BEGIN) || /<tableEdit\b/i.test(content) || /<tableEdit\b/i.test(reasoning);
    const code = codeFor(summary, found);
    globalThis.__memoNRelayDebug = { ...summary, foundMachineBlock: found, code };
    console.log('[Memo-N][debug] 中转站本轮结果', globalThis.__memoNRelayDebug);
    setTimeout(() => EDITOR.info(`Memo-N诊断：${code}`,'',5000), 250);
}
...
console.log('[Memo-N][debug] 手机诊断Toast已加载；P=中转站 T=dataTable含机器协议 E=system记录协议 U=user强化 J=json_schema残留 O=实际输出机器块');
