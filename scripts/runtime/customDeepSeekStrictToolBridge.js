            ...
            items: {
                ...
                properties: {
                    ...
                    expected: {
                        ...
                        description: '表2/4/5的update/delete必须原样填写当前目标row第一列原值；其他操作填null。',
                    },
                    cells: {
                        type: 'array',
                        items: {
                ...
                required: ['op', 'table', 'row', 'expected', 'cells'],
            },
        },
    },
...
function rewriteUserReminder(content) {
    ...
    return `${clean}\n\n[Memo-N本轮唯一交付：思考完成后调用一次${TOOL_NAME}。reply必须包含本来应有的状态栏、正文、行动选项和其他可见结构；changes每项都填写expected，表2/4/5的update/delete写当前行第一列原值，其余写null；无变化为[]。不要直接输出最终正文，也不要输出tableEdit/JSON等第二套机器格式。]`;
}
