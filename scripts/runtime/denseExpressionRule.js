...
const PATCH_MARK = '__memoNDenseExpressionRuleV11';
...
const TABLE_RULE = '七表遵循“每条事实先确定唯一主位置”的原则：先判断事实属于场景快照、玩家资料/状态、当前持有物、未结束事项、NPC稳定识别信息、NPC最新发展锚点，还是0～5没有合适字段但后续剧情需要知道的既成事件/连续性事实，再写入对应主位置；同一事实已有专门字段或专门表时，其他位置只保留必要关联，不整句重复搬运。表0保存玩家下次输入前的最终场景快照；表1保存玩家本人当前状态、稳定能力与持续可识别资料；表2保存玩家当前实际持有物；表3保存尚未结束的任务、承诺、交易或约定；表4保存NPC稳定身份、关系、外貌、性格及长期识别信息；表5保存NPC最新发展锚点，其中“主要能力”写实际能力，“当前状态”写当前处境或状态，“主要目标/重要事项”写当前目标或需持续追踪事项；表6既保存重大历史节点，也承接0～5没有合适字段但后续剧情需要知道的已发生事实，如异常发现、关键交互、决定、线索、关系转折或连续事件。维护时按事实性质判断主位置，发现错栏或跨表重复就迁回主位置并合并去重，同时保留有效信息。表格记录可确认事实而不是叙事文风；一段话中同时包含多类信息时应拆分判断，而不是整句搬入某个字段。';
const UPDATE_RULE = '更新方式按数据性质判断，而不是给每个字段死记一种操作：当前值发生变化时覆盖为最新值；持续能力、身份/所属、别名/称号、外貌等稳定资料以合并维护为主，同一事实被更准确地细化、替代或纠正时更新原内容；独立对象按新增、变化、失效分别insert/update/delete；历史事件按事件连续性维护，新独立事件新增，同一连续事件优先更新已有记录，重复或明确错误时合并、纠正或删除。';
...
function upgradeDefaultStepPrompt(value) {
    ...
    const oldAnchor = '表5字段“年龄”和“最后确认时间”必须分开：年龄是人物属性，最后确认时间是该发展锚点最后被剧情确认的世界时间；未知分别留空。';
    const anchor = '表5字段“年龄”和“最后确认时间”必须分开：年龄是人物属性，最后确认时间是该发展锚点最后被剧情确认的世界日期，精确到日即可；未知分别留空。';
    if (text.includes(oldAnchor)) text = text.replaceAll(oldAnchor, anchor);
...
function install() {
    ...
    patchCustomApi();
    console.log('[Memo-N] 七表字段语义规则已加载：按事实性质判断主位置，表6按重大节点+必要连续性维护');
}
