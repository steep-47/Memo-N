...
const PATCH_MARK = '__memoNDenseExpressionRuleV4';
...
const STEP_MARKER = '[Memo七表独立记录v8-七表字段语义边界]';
...
function installPromptRules() {
    ...
    if (upgradedStep && upgradedStep !== currentStep) {
        ...
        changed = true;
        console.log('[Memo-N][field-semantics] 已升级手动更新默认提示：按七表主位置判断字段归属');
    }
    ...
    if (upgradedBase && upgradedBase !== currentBase) {
        ...
        changed = true;
        console.log('[Memo-N][field-semantics] 已给正常记录补充七表字段归属与更新判断');
    }
...
function install() {
    ...
    patchCustomApi();
    console.log('[Memo-N] 七表字段语义规则已加载：先判定事实性质与主位置，再决定记录和更新方式');
}
