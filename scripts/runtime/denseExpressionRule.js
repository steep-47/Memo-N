...
const PATCH_MARK = '__memoNDenseExpressionRuleV9';
...
const APPEARANCE_RULE = '外貌特征只保留已确认、稳定且能明显帮助识别人物的外形信息；先提取最有辨识度的整体特征和关键细节，普通、常见、相近的信息合并或省略。当现有信息已足以形成清晰人物印象时停止继续补充。表达保持紧凑，在不丢失关键位置、程度和状态的前提下优先减少低辨识度细节。';
...
function install() {
    ...
    patchCustomApi();
    console.log('[Memo-N] 七表字段语义规则已加载：按事实性质判断主位置，外貌按辨识度优先并在足够时收束');
}
