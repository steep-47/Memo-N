...
function tableNameForIndex(tableIndex) {
    ...
    return String(visiblePromptSheets()[index]?.name ?? '').trim() || null;
}
...
function extractCalls(text) {
    ...
    while (i < source.length) {
        const regex = /(insertRow|updateRow|deleteRow)\s*\(/g;
        regex.lastIndex = i;
        ...
        const argsText = source.slice(regex.lastIndex, cursor);
        let args;
        try { args = JSON5.parse(`[${quoteNumericObjectKeys(argsText)}]`); }
        catch (error) { return { ok:false, error:`${name} 参数无法解析：${error?.message || error}`, calls:[], residue:'' }; }
...
function validateData(data, columnCount, label) {
    ...
    for (const key of keys) {
        const column = strictIndex(key);
        if (column === null || column >= columnCount) return { ok:false, error:`${label} 列${key}无效或越界` };
...
function validateAction(call) {
    ...
    const expected = call.name === 'updateRow' ? 3 : 2;
    if (args.length !== expected) return { ok:false, error:`${call.name}参数数量必须为${expected}，实际为${args.length}` };
    const [tableIndex, arg1, arg2] = args;
    const index = strictIndex(tableIndex);
    if (index === null) return { ok:false, error:`${call.name} tableIndex必须是非负安全整数` };
    const sheet = sheetForIndex(index);
    if (!sheet) return { ok:false, error:`#${index} ${tableNameForIndex(index) || '未知/不可见表'} 不存在；当前可写表共${visiblePromptSheets().length}张` };
    const columnCount = Math.max(0, Number(sheet.getHeader?.().length) || 0);
    const rowCount = Math.max(0, (Number(sheet.getRowCount?.()) || 0) - 1);
    if (call.name === 'insertRow') {
        const checked = validateData(arg1, columnCount, `insertRow(${index})`);
        return checked.ok ? { ok:true, action:{ type:'insert', tableIndex:index, sheet, data:arg1 } } : checked;
    }
    const rowIndex = strictIndex(arg1);
    if (rowIndex === null || rowIndex >= rowCount) return { ok:false, error:`${call.name}(${index}) rowIndex=${arg1}无效或越界；当前数据行数=${rowCount}` };
    if (call.name === 'deleteRow') return { ok:true, action:{ type:'delete', tableIndex:index, sheet, rowIndex } };
    const checked = validateData(arg2, columnCount, `updateRow(${index},${rowIndex})`);
    return checked.ok ? { ok:true, action:{ type:'update', tableIndex:index, sheet, rowIndex, data:arg2 } } : checked;
}
function validateBatch(actions) {
    ...
    for (const action of actions) {
        ...
        if (!columns) { columns = new Set(); updatedColumns.set(key, columns); }
...
function snapshotSheets(sheets) {
    ...
    for (const sheet of sheets) {
        ...
        if (!data || typeof data !== 'object') throw new Error(`表格 ${sheet?.name || '未知表'} 不支持完整序列化`);
...
function restorePieceState(piece, state) {
    ...
    if (state.hadHash) piece.memo_n_hash_sheets = copyHash(state.hash); else delete piece.memo_n_hash_sheets;
...
export function executeMemoTableEdit(raw, piece = null) {
    ...
    let snapshots;
    try { snapshots = snapshotSheets(touched); }
    catch (error) { return { ok:false, changed:false, noChange:false, count:0, error:`建立事务快照失败：${error?.message || error}` }; }
    try {
        ...
        restorePieceState(targetPiece, pieceState);
        const suffix = rollbackFailures.length ? `；回滚异常：${rollbackFailures.join('；')}` : '';
        return { ok:false, changed:false, noChange:false, count:0, error:`${error?.message || error}${suffix}` };
    }
}

export function getMemoWritableSheets() {
    return visiblePromptSheets();
}
