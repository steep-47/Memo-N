...
function coordinateRelayPrompt(data) {
    ...
    console.log(`[Memo-N] 中转站提示已协调：不再修改最后user消息｜tablePrompt=${rewritten}｜recordContract=${contracts}｜source=${endpoint.source || 'unknown'}｜customUrl=${endpoint.customUrl}｜reverseProxy=${endpoint.reverseProxy}`);
}
...
console.log('[Memo-N] 中转站提示协调器已加载：仅协调dataTable与system记录协议，不再改写用户消息；直连不变');
