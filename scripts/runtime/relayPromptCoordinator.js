...
function coordinateRelayPrompt(data) {
    ...
    if (rewritten) {
        console.log(`[Memo-N] 中转站提示已协调：移除JSON/tableEdit冲突｜count=${rewritten}｜source=${endpoint.source || 'unknown'}｜customUrl=${endpoint.customUrl}｜reverseProxy=${endpoint.reverseProxy}`);
    } else {
        console.warn('[Memo-N] 中转站提示协调器未找到dataTable主提示；保留recordEngine末尾tableEdit协议作为兜底');
    }
}
...
console.log('[Memo-N] 中转站提示协调器已加载：仅中转站改写dataTable输出段，直连不变');
