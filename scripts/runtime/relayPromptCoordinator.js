...
function coordinateRelayPrompt(data) {
    if (!data || typeof data !== 'object' || getProviderRoute(data) !== ROUTE.RELAY || !Array.isArray(data.messages)) return;
    ...
    console.log(`[Memo-N] 非CUSTOM中转站提示已协调｜tablePrompt=${rewritten}｜recordContract=${contracts}｜source=${info.source || 'unknown'}`);
}
...
console.log('[Memo-N] 中转站提示协调器已加载：只处理非CUSTOM relay；DeepSeek/CUSTOM不进入此模块');
