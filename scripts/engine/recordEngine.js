...
#1 角色状态表：只记录玩家本人。检查姓名、性别、种族、年龄、修为、灵根/体质、灵力、神识、身体状态、灵石、钱财、技能/术法、擅长、其他状态等明确现值。除本轮新确认或变化外，表中字段为空而当前上下文已有明确现值时也补齐；正文状态栏出现的已确认字段逐项核对。数字0是有效值，不当作空白。未知信息留空，不根据名字、外貌或常识猜测。
#2 背包表：检查玩家实际获得、消耗、丢失、交付、数量变化、品质/状态变化的物品，也补录表中缺失但当前明确仍持有的物品。普通工具、文书、材料和生活物资只要需要保持数量或持有连续性，也属于库存；已经失去且不再持有的项目按现有表语义更新或删除。
