...
import { executeMemoTableEdit, restoreMemoSnapshot, saveMemoSnapshot } from '../runtime/safeTableExecutor.js?v=memon80';
import {
...
#1 角色状态表：只记录玩家本人。检查姓名、性别、种族、年龄、修为、灵根/体质、灵力、神识、身体状态、灵石、钱财、技能/术法、擅长、其他状态等明确现值。灵石、钱财保存钱财戳中的货币余额；灵力、神识、身体状态及其他状态保存状态戳中的持续资源与当前状态。除本轮新确认或变化外，表中字段为空而当前上下文已有明确现值时也补齐；正文钱财戳、状态戳出现的已确认字段逐项核对。数字0是有效值，不当作空白。未知信息留空，不根据名字、外貌或常识猜测。
