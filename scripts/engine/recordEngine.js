...
const USER_REMINDER = '[Memo-N记录提醒]';
...
function relayMachineRule() {
    return `回复第一段必须先输出一个完整<tableEdit>机器记录块，闭合后继续完整执行原有# 输出要求。<tableEdit>内部只写函数调用文本：insertRow(tableIndex,{columnIndex:"value",...})、updateRow(tableIndex,rowIndex,{columnIndex:"value",...})、deleteRow(tableIndex,rowIndex)，或NO_CHANGE。不要使用XML/HTML字段标签。机器记录不能替代、删减或重排原有正文、状态、时间、金钱、选项及角色内容。`;
}
function reinforceRelayTablePrompt(messages) {
    ...
    for (const message of messages) {
        ...
        if (at >= 0) {
            ...
            message.content = `${head}\n- ${relayMachineRule()}${originalOutput}`;
        } else message.content = `${text.trimEnd()}\n\n# Memo-N机器记录补充\n${relayMachineRule()}`;
        count++;
    }
    return count;
}
function reinforceRelayLastUser(messages) {
    for (let i = messages.length - 1; i >= 0; i--) {
        ...
        if (!message.content.includes(USER_REMINDER)) message.content = `${message.content.trimEnd()}\n\n${USER_REMINDER}：回复第一段先给完整<tableEdit>机器块，块内只写insertRow/updateRow/deleteRow函数调用或NO_CHANGE；闭合后完整执行原有回复要求。`;
        return true;
    }
    return false;
}
...
function relayContract() {
    return `${MARKER}\n[Memo-N中转站唯一记录协议]\n${relayMachineRule()}\n有操作就写操作；没有操作也必须输出<tableEdit><!-- NO_CHANGE --></tableEdit>。只有当前表格里真实存在的rowIndex才能用于updateRow/deleteRow；空表首次记录只能insertRow。\n${sharedRecordRules()}`;
}
...
function inject(data) {
    ...
    pending = { at: Date.now(), type: armed.type, session, startLength: Array.isArray(session) ? session.length : 0, base, baseMes: String(base?.mes ?? ''), baseSwipeId: Number(base?.swipe_id ?? -1), baseReasoning: base ? reasoningText(base) : '', responseMode: relayMode ? 'relay_tableedit' : 'json', route };
    armed = null;
    let tablePromptReinforced = 0; let lastUserReinforced = false;
    if (Array.isArray(data.messages)) {
        data.messages = data.messages.filter(message => !String(message?.content ?? '').includes(MARKER));
        if (relayMode) { tablePromptReinforced = reinforceRelayTablePrompt(data.messages); lastUserReinforced = reinforceRelayLastUser(data.messages); }
    ...
    globalThis.__memoNLastRequestProbe = Object.freeze({ at: Date.now(), route, responseMode: relayMode ? 'relay_tableedit_three_point' : 'json', messageCount: messages.length, markerPresent: messages.some(message => String(message?.content ?? '').includes(MARKER)), relayTableEditPresent: relayMode && messages.some(message => String(message?.content ?? '').includes('<tableEdit>')), tablePromptReinforced, recordTaskReinforced: relayMode, lastUserReinforced, finalRole: String(last?.role ?? ''), provider: info?.route ?? route });
}
...
async function unpack(chatId) {
    ...
    const chat = USER?.getContext?.()?.chat?.[chatId]; if (!chat || chat.is_user || handled.get(chat) === chat.mes) return false; let current = String(chat.mes ?? ''); const isAppend = appendType(job.type) && job.base === chat && job.baseMes && current.startsWith(job.baseMes); const waited = await waitForCompleteEnvelope(chat, job, isAppend); if (waited.detached) { pending = null; return false; } current = waited.current; const envelope = waited.envelope; pending = null;
    ...
    const baselineSnapshot = copySnapshot(isAppend ? chat.memo_n_hash_sheets : previousSnapshot(chatId)); const baseline = isAppend ? { ok: !!baselineSnapshot, error: baselineSnapshot ? '' : 'Continue缺少当前表格基线' } : restoreMemoSnapshot(baselineSnapshot); const executionInput = envelope.tableEdit ? envelope.tableEdit : changesToStrictCalls(envelope.changes); const execution = baseline.ok ? executeMemoTableEdit(executionInput, chat) : { ok: false, changed: false, noChange: false, count: 0, error: baseline.error }; setStatus(chat, envelope, execution);
...
console.log('[Memo-N] 一次API记录引擎已加载：中转站三点协议由recordEngine单一所有');