...
const PROMPT_MARKER = '[Memo-N YiYi preset memory bridge v3]';
const START = '<yiyiMemory>';
const END = '</yiyiMemory>';
const PRESET_MARKERS = ['你叫伊依，是陪伴','伊依独立长期记忆数据','伊依始终在后台陪着'];
const RECORD_ONLY_MARKERS = ['[Memo当前表格独立记录v5]', '[Memo七表独立记录v4]', '[Memo七表独立记录v3]', '[Memo七表整理', '世界状态数据库整理器', 'Memo世界状态表格整理器', '只维护表格，不输出剧情正文'];
...
function hasYiYiPreset(messages) {
    return Array.isArray(messages) && messages.some(message => PRESET_MARKERS.some(marker => contentOf(message).includes(marker)));
}
...
function inject(data) {
    ...
    data.messages = data.messages.filter(message => !contentOf(message).includes('[Memo-N YiYi preset memory bridge'));
    data.messages.push({ role:'system', content:contract(buildYiYiRecallContext({ query:recentQuery(data.messages), maxMemories:10, maxChars:900 })) });
...
async function processLatest() {
    ...
    try {
        ...
        const initialToken = `${Number(piece.swipe_id ?? 0)}\u241f${String(piece.mes ?? '')}`;
        if (handled.get(piece) === initialToken) return;
        ...
        if (chat !== USER?.getContext?.()?.chat) return;
        const parsed = parseBlock(piece.mes);
        if (!parsed) return;
        ...
        if (parsed.error) {
            ...
            try {
                ...
                globalThis.__memoNYiYiPresetBridge = { at:Date.now(), injected:true, written:true, changed:result.changed || maintained.changed, total:maintained.stats?.total ?? maintained.vault?.memories?.length ?? 0 };
            ...
            } catch (error) { console.error('[Memo-N][伊依] 预设模式长期记忆写入失败', error); }
        }
...
function scheduleProcessLatest() {
    queueMicrotask(() => void processLatest().catch(error => console.error('[Memo-N][伊依] 预设模式记忆处理异常', error)));
}
...
APP.eventSource.on(endEvent, scheduleProcessLatest);
APP.eventSource.makeLast?.(endEvent, scheduleProcessLatest);
console.log('[Memo-N][伊依] 预设角色桥已加载：当前世界表与伊依独立长期记忆协议已同步');
