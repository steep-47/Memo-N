...
import { rebuildSheets } from "../runtime/absoluteRefresh.js?v=memon3";
...
export function cellClickEditModeEvent(cell) {
    ...
    if (cell.type === Cell.CellType.row_header) {
        ...
        const isPersonSheet = ['人物主表', '人物发展表'].includes(cell.parent?.name);
        const nameCell = isPersonSheet ? cell.parent.getCellsByRowIndex?.(cell.position[0])?.[1] : null;
        const characterName = String(nameCell?.data?.value ?? '').trim();
        const pinnedList = USER?.getSettings?.()?.memo_n_settings?.pinned_character_names || [];
        const isPinned = Number.isFinite(pinRank(characterName, pinnedList));
        const pinDiv = isPersonSheet && characterName
            ? $(`<div><i class="menu_button menu_button_icon interactable fa-solid fa-thumbtack" title="${isPinned ? '取消置顶' : '置顶人物'}" style="margin:0;border:none;color:${isPinned ? 'var(--SmartThemeQuoteColor)' : 'var(--SmartThemeEmColor)'}"></i></div>`)
            : null;
        ...
        pinDiv?.on('click', (e) => {
            ...
            if (!settings?.memo_n_settings) return;
            settings.memo_n_settings.pinned_character_names = toggleCharacterPin(characterName, settings.memo_n_settings.pinned_character_names);
            USER.saveSettings?.();
            updateSystemMessageTableStatus();
            renderSheetsDOM();
            EDITOR.info(Number.isFinite(pinRank(characterName, settings.memo_n_settings.pinned_character_names)) ? `已置顶：${characterName}` : `已取消置顶：${characterName}`);
        });
        ...
        if (pinDiv) $(rightDiv).append(pinDiv)
