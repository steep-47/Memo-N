...
import { pinRank } from '../engine/characterPins.js';
...
async function renderEditableSheetsDOM(_sheets, _viewSheetsContainer) {
    ...
    if (sumAlternateLevel > 0) {
        ...
        indexedTable.sort((a, b) => {
            ...
            const pinned = USER?.getSettings?.()?.memo_n_settings?.pinned_character_names || [];
            const rankA = pinRank(roleA, pinned);
            const rankB = pinRank(roleB, pinned);
...
export function updateSystemMessageTableStatus(force = false) {
    ...
    const sheets = BASE.hashSheetsToSheets(BASE.getLastSheetsPiece()?.piece.memo_n_hash_sheets);
...
export async function openTableRendererPopup() {
    ...
    const sheetsData = BASE.getLastSheetsPiece()?.piece.memo_n_hash_sheets;
    if (!sheetsData) {
