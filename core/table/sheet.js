...
export class Sheet extends SheetBase {
    ...
    renderSheet(cellEventHandler = this.lastCellEventHandler, targetHashSheet = this.hashSheet, lastCellsHashSheet = null) {
        ...
        // 仅对数组做本地浅拷贝；不要调用 BASE.copyHashSheets（它面向 memo_n_hash_sheets 映射对象，不是二维数组）
        ...
        // 1) 获取上一轮的 memo_n_hash_sheets（优先使用参数；若无参数且存在渲染上下文，则自动计算）
        ...
        if (prevHashSheetsMap === null && typeof DERIVED?.any?.renderDeep === 'number' && DERIVED.any.renderDeep !== 0) {
            try {
                prevHashSheetsMap = BASE.getLastSheetsPiece(DERIVED.any.renderDeep - 1, 3, false)?.piece?.memo_n_hash_sheets;
    ...
    save(targetPiece = USER.getChatPiece()?.piece, manualSave = false) {
        const hadTargetHash = !!targetPiece && Object.prototype.hasOwnProperty.call(targetPiece, 'memo_n_hash_sheets');
        let previousContext;
        let previousTargetHash;
        try {
            previousContext = structuredClone(BASE.sheetsData.context ?? []);
            previousTargetHash = hadTargetHash ? structuredClone(targetPiece.memo_n_hash_sheets) : undefined;
            ...
            if (!targetPiece) {
                console.log("没有消息能承载memo_n_hash_sheets数据，不予保存")
                return this
            }
            if (!targetPiece.memo_n_hash_sheets) targetPiece.memo_n_hash_sheets = {};
            targetPiece.memo_n_hash_sheets[this.uid] = this.hashSheet?.map(row => row.map(hash => hash));
            ...
            if (targetPiece) {
                if (hadTargetHash) targetPiece.memo_n_hash_sheets = previousTargetHash;
                else delete targetPiece.memo_n_hash_sheets;
            }
