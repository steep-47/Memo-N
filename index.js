...
import { TableTwoStepSummary } from "./scripts/runtime/separateTableUpdate.js?v=memon81";
...
import { initRefreshTypeSelector } from './scripts/runtime/absoluteRefresh.js?v=memon81';
...
import { executeMemoTableEdit, restoreMemoSnapshot } from './scripts/runtime/safeTableExecutor.js?v=memon81';
import { getMemoTableEditChannel } from './scripts/runtime/memoResponseChannels.js?v=memon81';
import { initExternalDataAdapter } from './external-data-adapter.js';


console.log("______________________记忆插件：开始加载______________________")

const VERSION = '0.1.0-memon.81'
...
export function buildSheetsByTemplates(targetPiece) {
    ...
    // USER.getChatPiece().memo_n_hash_sheets = {};
...
export function getTablePromptByPiece(piece, isPureData = false) {
    const {memo_n_hash_sheets} = piece
    const restored = restoreMemoSnapshot(memo_n_hash_sheets)
...
export function parseTableEditTag(piece, mesIndex = -1, ignoreCheck = false) {
    ...
    const restored = restoreMemoSnapshot(prePiece?.memo_n_hash_sheets)
...
async function onMessageReceived(chat_id) {
    if (globalThis.__memoNRecordEngineActive === true && USER.tableBaseSetting.step_by_step !== true) return
...
async function onChatChanged() {
    try {
        ...
        for (const sheet_id in piece.memo_n_hash_sheets) {
            if (piece.memo_n_hash_sheets[sheet_id].length > 1) {
                isDataEmpty = false;
                break;
            }
        }
...
jQuery(async () => {
    ...
    // Memo-N独立版本标识；不复用原Memo的更新状态。
    $("#tableUpdateTag").show().text(`v${VERSION}`);
    $("#table_message_tip").text("Memo-N 独立世界状态记忆");
