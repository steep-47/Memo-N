...
- 表0记录玩家下次输入前的最终日期、时间、地点和场景人物；正文经过多个时段或地点时记录最后落点，不照抄开头状态栏。只有七表当前事实全部一致且没有待补漏项时changes才为空数组。`,
  ...
  {"role":"user","content":"<参考上下文>\\n$1\\n</参考上下文>\\n<本轮待记录正文>\\n$2\\n</本轮待记录正文>\\n<世界书参考>\\n$4\\n</世界书参考>\\n只记录本轮已经明确发生或确认的变化；当前表格是行存在与rowIndex的唯一依据，旧聊天和历史操作不得用来猜行号，空表只insert；钱财/灵石保存货币余额，持续资源与生效状态保存到角色状态相应字段，普通物品只进背包且不重复；已有对象优先update，禁止重复insert；不同种族/体系的修为保留原生境界文本，不自动换算成人族境界。"}
]`,
    ...
    tableStructure: [
        ...
        {tableName:'角色状态表',tableIndex:1,columns:['姓名','性别','种族','年龄','修为','灵根/体质','灵力','神识','身体状态','灵石','钱财','技能/术法','擅长','其他状态','外貌特征'],enable:true,Required:true,asStatus:true,toChat:true,note:'<user>/玩家本人专属实时状态表，只允许一行；钱财/灵石保存钱财戳中的货币余额，灵力/神识/身体状态/其他状态保存状态戳中的持续资源与生效状态；外貌特征保存已确认的稳定外观和持久变化；数字0也是有效值；“修为”保留玩家原生体系表达',initNode:'首次得到玩家明确状态时插入；已有玩家行存在空字段且当前上下文能确认现值时补齐；钱财戳、状态戳和已确认的玩家外貌必须核对',insertNode:'仅当表为空且对象明确为<user>/玩家本人时插入',updateNode:'仅更新<user>/玩家本人；货币余额、持续资源、状态或外貌发生明确变化时按当前值覆盖，空字段按已确认现值补齐；普通物品不写入本表',deleteNode:'重复玩家状态行只保留最新有效一行'},
