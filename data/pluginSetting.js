...
# 输出
- Memo-N会在最终请求阶段提供唯一的一次API记录协议。先输出本轮事实变化changes记录块，再正常完成正文。
...
- 日期、时间、地点、当前场景人物任一发生变化（包括“日影移动”“日头升高”“片刻后”“随后”等明确时间推进）时必须维护表0；七表均无变化时changes为空数组。`,
  ...
  {"role":"user","content":"<参考上下文>\\n$1\\n</参考上下文>\\n<本轮待记录正文>\\n$2\\n</本轮待记录正文>\\n<世界书参考>\\n$4\\n</世界书参考>\\n只记录本轮已经明确发生或确认的变化；已有对象优先update，禁止重复insert；不同种族/体系的修为保留原生境界文本，不自动换算成人族境界。"}
    ...
    tableStructure: [
        ...
        {tableName:'角色状态表',tableIndex:1,columns:['姓名','性别','种族','年龄','修为','灵根/体质','灵力','神识','身体状态','灵石','钱财','技能/术法','擅长','其他状态'],enable:true,Required:true,asStatus:true,toChat:true,note:'<user>/玩家本人专属实时状态表，只允许一行；禁止记录任何NPC；“修为”记录玩家自身体系的原生境界/阶段，不换算成人族境界',initNode:'首次得到<user>/玩家本人的明确状态信息时插入',insertNode:'仅当表为空且对象明确为<user>/玩家本人时插入',updateNode:'仅更新<user>/玩家本人；当前值覆盖旧值；修为字段保留原生体系表达',deleteNode:'重复玩家状态行只保留最新有效一行'},
        ...
        {tableName:'人物主表',tableIndex:4,columns:['姓名','性别','种族/血脉','修炼体系/路径','别名/称呼','身份/所属','外貌特征','性格','与玩家关系','重要信息'],enable:true,Required:true,asStatus:true,toChat:true,note:'NPC专属身份与关系主表；负责识别人物是谁，并保存种族/血脉与修炼体系/路径；同一NPC一行；禁止记录<user>/玩家本人',initNode:'只记录后续值得继续引用的NPC；种族/血脉、修炼体系/路径及其他字段均只填已明确确认的信息，未知留空',insertNode:'出现新的重要NPC且表中没有时插入；不得为了补齐字段编造未知信息',updateNode:'正式姓名、种族/血脉、修炼体系/路径、身份/所属、外貌、性格、关系或长期重要信息发生明确变化时更新同一NPC行；称呼变化不重复建人',deleteNode:'重复NPC行删除并合并；若误写<user>/玩家本人则删除；NPC死亡通常保留人物主记录'},
