...
- 表0记录玩家下次输入前的最终日期、时间、地点和场景人物；正文经过多个时段或地点时记录最后落点，不照抄开头状态栏。只有七表当前事实全部一致且没有待补漏项时changes才为空数组。`,
    ...
    refresh_system_message_template: `你是世界状态表格整理助手。只根据已确认事实维护现有七张表。优先更新已有行，不写流水账，不猜测未知。玩家货币余额写角色状态表的钱财/灵石，持续资源与生效状态写角色状态表相应字段，普通物品只写背包且不与货币重复。人物主表保存NPC识别信息以及已确认的种族/血脉、修炼体系/路径；人物发展表保存最新发展锚点，其中“修为”必须保留角色自身体系的原生境界文本，禁止换算成人族境界；年龄与最后确认时间分开维护；历史表只保存影响未来推演的重要节点。只输出<tableEdit>。`,
    refresh_user_message_template: `<聊天记录>\n$1\n</聊天记录>\n<当前表格>\n$0\n</当前表格>\n<表头信息>\n$2\n</表头信息>\n按0当前状态→1角色状态→2背包→3任务约定→4人物主表→5人物发展表→6历史事件检查。钱、金银铜钱、通用灵石等货币余额只进角色状态表；灵力、神识、体力等持续状态进角色状态表相应字段；普通物品只进背包，不跨表重复。同一对象已有行优先update；人物主表保存NPC种族/血脉与修炼体系/路径等稳定事实；人物发展表“修为”只记录其原生体系境界，不把战力对应换算成人族境界；重要节点才写历史；不猜测未知。函数放在<tableEdit><!-- ... --></tableEdit>中。`,
  ...
  {"role":"user","content":"<参考上下文>\\n$1\\n</参考上下文>\\n<本轮待记录正文>\\n$2\\n</本轮待记录正文>\\n<世界书参考>\\n$4\\n</世界书参考>\\n只记录本轮已经明确发生或确认的变化；钱财/灵石保存货币余额，持续资源与生效状态保存到角色状态相应字段，普通物品只进背包且不重复；已有对象优先update，禁止重复insert；不同种族/体系的修为保留原生境界文本，不自动换算成人族境界。"}
]`,
    ...
    tableStructure: [
        ...
        {tableName:'角色状态表',tableIndex:1,columns:['姓名','性别','种族','年龄','修为','灵根/体质','灵力','神识','身体状态','灵石','钱财','技能/术法','擅长','其他状态'],enable:true,Required:true,asStatus:true,toChat:true,note:'<user>/玩家本人专属实时状态表，只允许一行；钱财/灵石保存钱财戳中的货币余额，灵力/神识/身体状态/其他状态保存状态戳中的持续资源与生效状态；数字0也是有效值；“修为”保留玩家原生体系表达',initNode:'首次得到玩家明确状态时插入；已有玩家行存在空字段且当前上下文能确认现值时补齐；钱财戳和状态戳中的已确认字段必须核对',insertNode:'仅当表为空且对象明确为<user>/玩家本人时插入',updateNode:'仅更新<user>/玩家本人；货币余额、持续资源或状态按当前值覆盖，空字段按已确认现值补齐；普通物品不写入本表',deleteNode:'重复玩家状态行只保留最新有效一行'},
